...
\theoremstyle{plain} % Native AMS parameter
\theoremstyle{definition} % Adjusts fonts automatically for descriptions
...
\author{
  \begin{tabular}{c}
    \textbf{James E. Rock} \\
    \small Independent Researcher, Ypsilanti, MI, USA \\
    \small ORCID: 0000-0003-2858-6077 \\
    \small Email: \texttt{jimrock69@comcast.net}
  \end{tabular}
}

\date{}
% Leaves this empty so the journal doesn't inject a default current date
\maketitle
\begin{abstract}
...
\noindent \textbf{Keywords:} Goldbach Conjecture, Symmetric Sieve Correlation (SSC), Symplectic-Vaughan Constant, Spectral Rigidity, Deterministic Sieve, Lean 4 formalization. \\
\noindent \textbf{2020 Mathematics Subject Classification:} 11P32, 11N35, 11N05, 42A16.
...
\begin{itemize}
    \item \textbf{Repository URL:} \url{https://github.com/jimrock69/GOLD-Constructive-Certificate}
...
Table \ref{table:scaling_data} reflects this refined model, calculated via the C++ program in Appendix B. By utilizing the $4/\ln^2 n$ factor, the prediction provides a significantly tighter fit to the actual Goldbach count $G(n)$ while maintaining its status as a deterministic floor.
...
\textbf{Human and AI-Collaboration:}
This research was developed through a unique ``Resonant Feedback Loop'' between human mathematical intuition and artificial intelligence. The core logic of the 
Symmetric Sieve Correlation (SSC) Algorithm originated 
with the author. The AI (Gemini \cite{gemini2025}) served as the formalization engine, translating the 
conceptual blueprint into rigorous mathematical prose and optimizing the structural hierarchy of the text. 

This manuscript is original work and is not under consideration for publication elsewhere. I received no funding and I have no conflicts of interest to disclose.
\raggedright
